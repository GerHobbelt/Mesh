...
// > mimalloc-test-stress [THREADS] [SCALE] [ITER]  [sleep]
//
// argument defaults
static int THREADS = 32;      // more repeatable if THREADS <= #processors
static int SCALE   = 10;      // scaling factor
static int ITER    = 50;      // N full iterations destructing and re-creating all threads
static int SLEEP   = 0;       // sleep very iteration
...
static void test_stress(void) {
  uintptr_t r = rand();
  bool can_fork = true;
  auto last_fork_check = std::chrono::high_resolution_clock::now();
  auto last_fork = std::chrono::high_resolution_clock::now();
  ...
  for (int n = 0; n < ITER; n++) {
...
#ifndef NDEBUG
    if ((n + 1) % 10 == 0) { printf("- iterations left: %3d,  use - %f\n", ITER - (n + 1), diff.count()); }
    std::this_thread::sleep_for(std::chrono::seconds(SLEEP));
#endif
    
    last_fork_check = std::chrono::high_resolution_clock::now();
    diff = last_fork_check - last_fork;

    if(diff.count() > 10 && can_fork  && !is_child) {
      auto begin_fork = std::chrono::high_resolution_clock::now();

      pid_t pid = fork();
      last_fork = std::chrono::high_resolution_clock::now();
      can_fork = false;
      ...
      {
        auto end_fork = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> diff = end_fork - begin_fork;
        printf("- fork used - %f  in parent child pid=%d\n",  diff.count(), pid);
    ...
    auto r = ((float)(std::rand() ) /(float)RAND_MAX) * 10000;

    if(diff.count() > r && !is_child) {
      printf("r=%f\n", r);
      can_fork = true;
    }
  }
